...
class MidiParser(object):
    ...
    def process(
        ...
        if audio_path is not None:
            logging.info(
                "Computing spectrogram from audio path: {0}".format(audio_path)
            )
...
def extract_spectrogram(
    ...
    spec_proc = FilteredSpectrogramProcessor(
        LogarithmicFilterbank, num_bands=num_bands, fmin=fmin, fmax=fmax
    )
...
def render_audio(
    ...
    cmd = "fluidsynth -F %s -O s16 -T flac %s %s" % (
        audio_path,
        sound_font_path,
        perf_midi_path,
    )
...
if __name__ == "__main__":
    ...
    for midi_file_path in glob.glob(pattern):
        ...
        spec_file_path = os.path.join(
            directory.replace("/audio", "/spec"), file_name + "_spec.npy"
        )
        onset_file_path = os.path.join(
            directory.replace("/audio", "/spec"), file_name + "_onsets.npy"
        )
